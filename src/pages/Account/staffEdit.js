...
const StaffEdit = () => {
    ...
    const [fields, setFields] = useState({
        adminPassword: "",      //tài khoản của admin
        newPassword: "",        //tài khoản mới của nhân viên
        confirmPassword: ""     //xác nhân tài khoản mới của nhân viên
    });
    ...
    const handleTabChange = (event, newValue) => {
        setValue(newValue);
        if (newValue === 0) {
            navigate(`/staffList/staffEdit/${id}/change-personal-info`);
        } else {
            navigate(`/staffList/staffEdit/${id}/change-password`);
        }
    };

    // change staff info (without password)
    const changeInput = (e) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            {   
                ...
                setIsLoading(false);
                context.setAlertBox({ open: true, error: true, msg: 'Tài khoản của bạn không thể thực hiện chức năng này' });
                return false;
            }
        } catch (error) {
            setIsLoading(false);
            ...
            <div className='right-content w-100'>
                ...
                <Box sx={{ borderBottom: 1, borderColor: 'divider' }} >
                    <Tabs value={value} onChange={handleTabChange}>
                        <Tab label="Thông tin nhân viên" component={Link} to={`/staffList/staffEdit/${id}/change-personal-info`}/>
                        <Tab label="Đổi mật khẩu" component={Link} to={`/staffList/staffEdit/${id}/change-password`}/>
                    </Tabs>
                </Box>
